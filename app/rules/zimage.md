# Z-Image Turbo — Model Rules

Distilled turbo model: **cfg 0.8-1.0**, negative prompt has almost no effect (keep it empty or
minimal). Default steps 11 (range 8-14). Natural-language prompts. Fast model.
...
```json
{
  ...
  "12": {"class_type": "KSampler", "inputs": {"model": ["1", 0], "positive": ["5", 0], "negative": ["6", 0], "latent_image": ["11", 0], "seed": "{SEED}", "steps": "{STEPS}", "cfg": 0.8, "sampler_name": "euler", "scheduler": "beta", "denoise": 1.0}},
...
- **img2img**: replace node "11" with LoadImage(image="{REF_IMAGE}") -> VAEEncode(pixels, vae ["3",0])
  and set KSampler denoise 0.4-0.75 (lower = closer to the reference).
